"""
Example script demonstrating parallel compilation of Triton attention kernels.
"""
...
# Import the inner attention function from the attention_tuning.py file
@triton.jit
def _attn_fwd_inner(
...
def main():
    parser = argparse.ArgumentParser(description="Parallel Triton Attention Kernel Compilation Example")
    ...
    parser.add_argument("--batch-size", type=int, default=20, help="Batch size for attention")
    ...
    arguments = [
        ...
        np.int32(query.stride(0)).item(), np.int32(query.stride(1)).item(), 
        np.int32(query.stride(2)).item(), np.int32(query.stride(3)).item(),
        np.int32(key.stride(0)).item(), np.int32(key.stride(1)).item(), 
        np.int32(key.stride(2)).item(), np.int32(key.stride(3)).item(),
        np.int32(value.stride(0)).item(), np.int32(value.stride(1)).item(), 
        np.int32(value.stride(2)).item(), np.int32(value.stride(3)).item(),
        np.int32(attn_mask.stride(0)).item(), np.int32(attn_mask.stride(1)).item(), 
        np.int32(attn_mask.stride(2)).item(), np.int32(attn_mask.stride(3)).item(), 
        np.int32(out.stride(0)).item(), np.int32(out.stride(1)).item(), 
        np.int32(out.stride(2)).item(), np.int32(out.stride(3)).item(),
        np.int32(batch_size * num_heads).item(),  # Z
        np.int32(num_heads).item(),  # q_numhead
        np.int32(num_heads).item(),  # kv_numhead
        np.int32(seq_len).item(),    # Q_CTX
        np.int32(seq_len).item(),    # KV_CTX
    ]
    ...
    results, successful_configs = parallel_compile_triton_kernel(
        kernel_name="attention_kernel",
        kernel_fn=attention_kernel,
        arguments=arguments,
        tune_params=single_config,
